...
BitmapView::BitmapView(const BRect &frame, const char *name,
 	...
 	fRemovableBitmap(false)
{
	...
	fNoPhotoWidths[2] = StringWidth("Picture");
...
BitmapView::Draw(BRect rect)
{
	if (fBitmap)
		...
		DrawString("Picture",fNoPhotoOffsets[2]);
...
BitmapView::MessageReceived(BMessage *msg)
{
		...
		if (msg->what == B_SIMPLE_DATA) {
			...
			if (msg->FindInt32("be:actions",&actions) == B_OK) {
				...
				if (response.what == B_MIME_DATA) {
					...
					if (roster->Translate(&memio,NULL,NULL,&bstream,
	...
	switch (msg->what) {
		case M_REMOVE_IMAGE: {
			BAlert *alert = new BAlert("","This cannot be undone. Remove the image?",
										"Remove","Cancel");
...
BBitmap *
BitmapView::GetBitmap(void) const
{
	return fBitmap;
}
...
bool
BitmapView::IsEnabled(void) const
{
	return fEnabled;
}
...
bool
BitmapView::IsFixedSize(void) const
{
	return fFixedSize;
}
...
bool
BitmapView::AcceptsDrops(void) const
{
	return fAcceptDrops;
}
...
bool
BitmapView::AcceptsPaste(void) const
{
	return fAcceptPaste;
}
...
bool
BitmapView::ConstrainsDrops(void) const
{
	return fConstrainDrops;
}
...
bool
BitmapView::IsBitmapRemovable(void) const
{
	return fRemovableBitmap;
}
...
BRect
BitmapView::ScaleRectToFit(const BRect &from, const BRect &to)
{
